        ...
        for (Mutable<ILogicalOperator> inputOpRef : op.getInputs()) {
            if (inputOpRef.getValue() instanceof AbstractBinaryJoinOperator) {
                ...
                if (cond.getExpressionTag() == LogicalExpressionTag.FUNCTION_CALL) {
                    ...
                    if (fcond.getArguments().size() > 1) {
                        for (Mutable<ILogicalExpression> arg : fcond.getArguments()) {
                            ILogicalExpression exp = arg.getValue();
                            if (exp.getExpressionTag() == LogicalExpressionTag.FUNCTION_CALL) {
                                AbstractFunctionCallExpression innercond = (AbstractFunctionCallExpression) exp;
                                if (innercond.hasAnnotation(TempExpressionAnnotation.class)) {
                                    needReplicateOp.put(op, index);
                                }
                            }
                        }
    ...
    public boolean rewritePost(Mutable<ILogicalOperator> opRef, IOptimizationContext context)
        ...
        if (needReplicateOp.isEmpty()) {
            ...
            while (it.hasNext()) {
                ...
                beforeExchange.setExecutionMode(AbstractLogicalOperator.ExecutionMode.LOCAL);
