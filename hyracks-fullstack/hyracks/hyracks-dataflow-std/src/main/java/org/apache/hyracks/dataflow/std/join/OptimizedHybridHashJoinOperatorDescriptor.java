...
public class OptimizedHybridHashJoinOperatorDescriptor extends AbstractOperatorDescriptor {
    ...
    private final int minBuildPartitions;
    ...
    public OptimizedHybridHashJoinOperatorDescriptor(IOperatorDescriptorRegistry spec, int memSizeInFrames,
            int inputsize0, int buildPartitions, double factor, int[] keys0, int[] keys1,
        ...
        this.minBuildPartitions = buildPartitions;
    ...
    private static int getNumberOfPartitions(int memorySize, int buildSize, int minBuildPartitions, double factor,
            int nPartitions) throws HyracksDataException {
        int minPartitions = minBuildPartitions > 0 ? minBuildPartitions : 20;
        int minimumNumberOfPartitions = Math.min(minPartitions, memorySize);
    ...
    public static class BuildAndPartitionTaskState extends AbstractStateObject {
        ...
        private ArrayList<OptimizedHybridHashJoin> partitionJoins = new ArrayList<>();
        private int maxLevel = 0;
        private String sb = "RESULT:";;
        ...
        public IOperatorNodePushable createPushRuntime(final IHyracksTaskContext ctx,
            ...
            return new AbstractUnaryInputSinkOperatorNodePushable() {
                ...
                public void open() throws HyracksDataException {
                    ...
                    state.numOfPartitions = getNumberOfPartitions(state.memForJoin, inputsize0, minBuildPartitions,
            ...
            IOperatorNodePushable op = new AbstractUnaryInputUnaryOutputOperatorNodePushable() {
                ...
                public void close() throws HyracksDataException {
                    ...
                    try {
                        ...
                        buildStats.addAll(state.hybridHJ.getBuildStats());
                        probeStats.addAll(state.hybridHJ.getProbeStats());
                        for (OptimizedHybridHashJoin hhj : state.partitionJoins) {
                            probeStats.addAll(hhj.getBuildStats());
                            probeStats.addAll(hhj.getProbeStats());
                        }
                        if (shivasLog) {
                            LOGGER.debug("BuildStats: " + buildStats.toString());
                            LOGGER.debug("ProbeStats: " + probeStats.toString());
                            state.sb += "," + buildStats.getWrites() / jobletCtx.getInitialFrameSize() + ","
                                    + buildStats.getReads() / jobletCtx.getInitialFrameSize() + ","
                                    + probeStats.getWrites() / jobletCtx.getInitialFrameSize() + ","
                                    + probeStats.getReads() / jobletCtx.getInitialFrameSize();
                            buildStats.addAll(probeStats);
                            LOGGER.debug("TotalStats: " + buildStats.toString());
                            LOGGER.debug("Max Level of HHJ: " + state.maxLevel);
                        }
                    } catch (Exception e) {
                        if (state.hybridHJ != null) {
                            state.hybridHJ.fail();
                ...
                private void joinPartitionPair(RunFileReader buildSideReader, RunFileReader probeSideReader,
                    ...
                    ITuplePartitionComputer buildHpc =
                            new FieldHashPartitionComputerFamily(buildKeys, buildHashFunctionFactories)
                                    .createPartitioner(level);
                    probeStats.addToReads(buildSideReader.getFileSize());
                    probeStats.addToReads(probeSideReader.getFileSize());
                ...
                private void applyHybridHashJoin(int tableSize, final String PROBE_REL, final String BUILD_REL,
                    ...
                    int n = getNumberOfPartitions(state.memForJoin, tableSize, minBuildPartitions, fudgeFactor,
                            nPartitions);
